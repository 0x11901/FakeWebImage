...
@property(nonatomic, strong)
    NSOperationQueue *queue; // a global concurrent queue
@property(nonatomic, strong)
    NSMutableDictionary *downloadingImages; // a global dictionary use for judge
                                            // the image download if needly
@property(strong, nonatomic)
    NSOperation *lastOperation; // a pointer to record last operation
@property(nonatomic, copy)
    NSString *lastURLString; // a pointer to record last URL
@property(nonatomic, strong)
    NSCache *memeryImages; // a NSCache for load images in memery
@end
...
- (instancetype)init {
  if (self = [super init]) {
    ...
    //        selector:@selector(removeAllMemeryCaches)
...
- (void)manger_donwloadImageWithURL:(NSString *)urlString
                       successBlock:(void (^)(UIImage *))successBlock {
  if (urlString != nil) {
    if ([_memeryImages objectForKey:urlString]) {
      ...
      //            NSLog(@"cancel last operation,download new
      //            image:%@",[urlString lastPathComponent]);
  ...
  _lastOperation = [DownloaderOperation
      donwloadImageWithURL:urlString
              successBlock:^(UIImage *image) {
                successBlock(image);
                [_memeryImages setObject:image forKey:urlString];
                [_downloadingImages removeObjectForKey:urlString];
                //        NSLog(@"remove the key for image(%@) which alraedy
                //        downloaded(%zd)",[urlString
                //        lastPathComponent],_downloadingImages.count);
              }];
