...
- (instancetype)initWithMethod:(Method)method {
  ...
  if (argumentCount > 0) {
    ...
    for (unsigned int i = 0; i < argumentCount; i++) {
      ...
      NSString *type =
          argumentType ? [NSString stringWithUTF8String:argumentType] : nil;
...
- (instancetype)initWithProperty:(objc_property_t)property {
  ...
  objc_property_attribute_t *attrs =
      property_copyAttributeList(property, &attrCount);
  for (unsigned int i = 0; i < attrCount; i++) {
    ...
    case 'G': {
      ...
      if (attrs[i].value) {
        _getter = NSSelectorFromString(
            [NSString stringWithUTF8String:attrs[i].value]);
      }
    } break;
    case 'S': {
...
- (void)_update {
  ...
  if (methods) {
    ...
    for (unsigned int i = 0; i < methodCount; i++) {
      YYClassMethodInfo *info =
          [[YYClassMethodInfo alloc] initWithMethod:methods[i]];
      if (info.name)
  ...
  if (properties) {
    ...
    for (unsigned int i = 0; i < propertyCount; i++) {
      YYClassPropertyInfo *info =
          [[YYClassPropertyInfo alloc] initWithProperty:properties[i]];
      if (info.name)
...
+ (instancetype)classInfoWithClass:(Class)cls {
  ...
  YYClassInfo *info =
      CFDictionaryGetValue(class_isMetaClass(cls) ? metaCache : classCache,
